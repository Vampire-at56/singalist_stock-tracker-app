...
/**
 * Combine class value arguments into a single merged CSS class string.
 *
 * @param inputs - Class value(s) (strings, arrays, or objects) to combine.
 * @returns A single class string with combined class names; Tailwind-style utility classes are merged to avoid conflicts.
 */
...
/**
 * Format a market capitalization value (in US dollars) into a compact, human-friendly string.
 *
 * @param marketCapUsd - Market capitalization in US dollars
 * @returns `'N/A'` if `marketCapUsd` is not finite or is less than or equal to zero; otherwise a USD-formatted string using `T` (trillions), `B` (billions), or `M` (millions) suffixes when applicable, or the full dollar amount with two decimal places
 */
